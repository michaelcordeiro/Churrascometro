...
function calcular() {
    ...
    // console.log(qtdTotalCarne + " Carne");
    
    let qtdTotalCerveja = cervejaPP(duracao) * adultos;
    // console.log(qtdTotalCerveja + " Cerveja");

    let qtdTotalBebida = bebidaPP(duracao) * adultos + (bebidaPP(duracao) / 2 * criancas);
    // console.log(qtdTotalBebida + " Bebidas");

    resultado.innerHTML = `<p>${qtdTotalCarne / 1000 } kg de Carne</p>`
    resultado.innerHTML += `<p>${Math.ceil(qtdTotalCerveja / 350)} Latas de Cerveja</p>`
    resultado.innerHTML += `<p>${Math.ceil(qtdTotalBebida / 2000)} Pet's 2l de Bebida (Refri/água)</p>`
